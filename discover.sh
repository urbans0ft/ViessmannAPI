#!/bin/bash
installationId=$(source ./get.sh "/iot/v1/equipment/installations")
...
#deviceId=$(source ./get.sh "/iot/v1/equipment/installations/$installtionId/gateways/$gatewaySerial/devices" | jq -r '.data[0].id')
